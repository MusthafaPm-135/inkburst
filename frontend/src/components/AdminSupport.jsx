...
function AdminSupport() {
    ...
    const sendReply = async (event) => { event.preventDefault(); const message = reply.trim(); if (!message || !selectedId) return; try { await API.post(`/support/admin/chats/${selectedId}/messages`, { message }); setReply(""); await Promise.all([loadMessages(selectedId), loadChats()]); } catch { setError("Could not send the reply."); } };
    ...
    return <section className="admin-panel support-admin-panel">
        ...
        <div className="support-admin-layout">
            ...
            <div className="support-admin-conversation">{selected ? <><header><div><strong>{selected.username}</strong><small>Chat #{selected.id}</small></div>{selected.status !== "resolved" && <button className="secondary-button" type="button" onClick={resolveChat}>Mark resolved</button>}</header><div className="support-admin-messages">{messages.map((message) => <div className={message.sender_role} key={message.id}><strong>{message.sender_role === "admin" ? "Admin" : selected.username}</strong><p>{message.message}</p></div>)}</div>{selected.status !== "resolved" && <form onSubmit={sendReply}><input maxLength="2000" value={reply} onChange={(event) => setReply(event.target.value)} placeholder="Reply to customer…" /><button className="primary-button" type="submit">Send</button></form>}</> : <p className="support-select-prompt">Select a customer chat.</p>}</div>
        </div>
    </section>;
}
