...
function Register() {
    ...
    return <main className="account-page"><section className="account-card"><Link className="account-logo" to="/">● KEYRA COMICS</Link><p className="eyebrow">NEW READER</p><h1>Build your comic shelf.</h1><p className="account-copy">Create an account to purchase and read digital comics.</p>{error && <p className="form-error">{error}</p>}<form onSubmit={submit} className="account-form"><label>Name<input required value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} /></label><label>Email<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label>Password <small>(at least 6 characters)</small><input required minLength="6" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label><button className="store-button" disabled={busy}>{busy ? "Creating…" : "Create account"}</button></form><p>Already a reader? <Link to="/login">Log in</Link></p></section></main>;
}
